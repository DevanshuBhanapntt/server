                            ...
                            if server["next_patch_time"] and groupid == server["server_group_id"]: # need to be uncommented while testing live data
                                ...
                                if timediff.days == daysinfuture and servervalidationcheck == 1:
                                    ...
                                    if serverbooleanresult == 1:
                                        ...
                                        shortdescription = "[TR - Automation] Patching " + server["name"] + ", Linux"
                                        ...
                                        if "PrimaryChangeNumber" in groupnotes:
                                            ...
                                            shortdescription = "[TR - Automation] Patching " + server["name"] + ", Linux , PrimaryChangeNumber:  " + PrimaryChangeNumber
                                        else:
...
This change was approved under the parent change Nucleus Patching Automation ("""+ customername +""" - Linux) and (PROD Servers)The server will be unavailable during the restart of the server.
...
If uninstalling the patch does not restore services, the server will be restored from backup following documented DR plans."""
            ...
            if len(changescreated) > 0 and len(servernotincmdb) == 0:
                ...
                messagebody = """
Nucleus Patching Automation Change Creation: All changes created for ("""+ companyname +""" - Linux) group ("""+ groupname +""")
...
This automated message was sent to you at: """ + str((datetime.now()).astimezone(pytz.timezone('GMT'))) + """."""
                ...
                message['Subject'] = 'Nucleus Patching Automation Change Creation: All changes created for ('+ companyname +' - Linux) group ('+ groupname +')'

            elif len(changescreated) > 0 and len(servernotincmdb) > 0:
                #Send Email with the changes created and not created due to issues

                messagebody = """
Nucleus Patching Automation Change Creation: Issues with (""" + companyname + """- Linux) group ("""+ groupname +""")
...
This automated message was sent to you at: """ + str((datetime.now()).astimezone(pytz.timezone('GMT'))) + """."""
                ...
                message['TO'] = email.utils.formataddr(('Recipient', receiver_email))
                message['From'] = email.utils.formataddr(('Nucleus Patching Automation', sender_email))
                message['Subject'] = 'Nucleus Patching Automation Change Creation: Issues with ('+ companyname +' - Linux) group ('+ groupname +')'
            else:
                #Send Email with the changes created and not created due to issues

                messagebody = """
Nucleus Patching Automation Change Creation: No Changes Created for (""" + companyname + """- Linux) group ("""+ groupname +""")

This automated message was sent to you at: """ + str((datetime.now()).astimezone(pytz.timezone('GMT'))) + """."""
                message = MIMEText(messagebody)
                message['TO'] = email.utils.formataddr(('Recipient', receiver_email))
                message['From'] = email.utils.formataddr(('Nucleus Patching Automation', sender_email))
                message['Subject'] = 'Nucleus Patching Automation Change Creation: No Changes Created for ('+ companyname + '- Linux) group ('+ groupname +')'
